...
import staideltaEnvironment from '../util/staideltaEnvironment';
import staideltaConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!staideltaEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    staideltaConfig.loadConfig('mainnet');
    ...
    const createWindow = async () => {
      ...
      if (!staideltaConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !staideltaConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (staideltaConfig.manageDaemonLifetime()) {
        staideltaEnvironment.startStaiDeltaDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
          ...
          {
            label: i18n._(/* i18n */ { id: 'StaiDelta Blockchain Wiki' }),
            click: () => {
              openExternal(
                'https://github.com/STATION-I/staidelta-blockchain/wiki',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/STATION-I/staidelta-blockchain/releases',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://www.facebook.com/groups/staideltaglobalsupportgroup/',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal('https://twitter.com/staideltaglobal');
            },
          },
        ],
      },
    ...
    if (process.platform === 'darwin') {
      // StaiDelta Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'StaiDelta' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About StaiDelta Blockchain' }),
            click: () => {
              openAbout();
            },
          },
