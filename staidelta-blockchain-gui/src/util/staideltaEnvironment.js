...
const PY_FOLDER = '../staidelta/daemon';
...
const getStaiDeltaVersion = () => {
  let version = null;
  const exePath = getExecutablePath('staidelta');
  // first see if we can get a staidelta exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or staidelta is on the path
    try {
      version = child_process
