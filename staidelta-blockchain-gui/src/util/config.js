...
function loadConfig(net) {
  try {
    // check if STAIDELTA_ROOT is set. it overrides 'net'
    const config_root_dir =
      'STAIDELTA_ROOT' in process.env
        ? process.env.STAIDELTA_ROOT
        : path.join(os.homedir(), '.staidelta', net);
    ...
    const daemon_port = lodash.get(config, 'ui.daemon_port', 22322); // jshint ignore:line
