...
import { AlertDialog, Card, Flex } from '@staidelta/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const staideltaper_input = ip_parsed.limit;
    ...
    const staideltaper_value = Number.parseInt(Number(staideltaper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (staidelta per interval):{' '}
                  {mojo_to_staidelta_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const amount = staidelta_to_mojo(amount_input.value);
    const fee = staidelta_to_mojo(fee_input.value);
