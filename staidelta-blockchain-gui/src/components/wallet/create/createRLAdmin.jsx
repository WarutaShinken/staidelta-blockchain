...
import { AlertDialog } from '@staidelta/core';
import {
  Typography,
  Button,
  Box,
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      staideltaper_input.value === '' ||
      Number(staideltaper_input.value) === 0 ||
      !Number(staideltaper_input.value) ||
      isNaN(Number(staideltaper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const staideltaper = staidelta_to_mojo(staideltaper_input.value);
    const staideltaper_value = Number.parseInt(Number(staideltaper));
    ...
    // var fee = staidelta_to_mojo(fee_input.value);
