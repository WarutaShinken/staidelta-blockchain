...
import { mojo_to_staidelta } from '../../util/staidelta';
import {
  calculatePoolReward,
...
export default function Block() {
  ...
  const poolReward = mojo_to_staidelta(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_staidelta(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const staideltaFees = blockRecord.fees
    ? mojo_to_staidelta(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.staideltaexplorer.org/blockchain/puzzlehash/${blockRecord.pool_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: staideltaFees ? `${staideltaFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <LayoutMain title={<Trans>Block</Trans>}>
      <Card
        title={
          <Back variant="h5">
            <Trans>
              Block at height {blockRecord.height} in the StaiDelta blockchain
            </Trans>
          </Back>
        }
        action={
