...
from staidelta.cmds.units import units
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total staidelta farmed: {amounts['farmed_amount'] / units['staidelta']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['staidelta']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['staidelta']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'staidelta start wallet' and 'staidelta wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'staidelta wallet show'")
    else:
        print("Note: log into your key using 'staidelta wallet show' to see rewards for each key")

    if staking_detail == 0:
        print("For details on staking you should run 'staidelta farm summary -sd 1'")
    elif staking_detail == 1:
        print("For number of coins staked per address you should run 'staidelta farm summary -sd 2'")
    elif staking_detail == 2:
        print("For number of plots per address you should run 'staidelta farm summary -sd 3'")
