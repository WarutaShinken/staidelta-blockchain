...
from staidelta import __version__
from staidelta.consensus.coinbase import create_puzzlehash_for_pk
from staidelta.ssl.create_ssl import (
    ensure_ssl_dirs,
    generate_ca_signed_cert,
    get_staidelta_ca_crt_key,
    make_ca_cert,
    write_ssl_cert_and_key,
)
from staidelta.util.bech32m import encode_puzzle_hash
from staidelta.util.config import (
    create_default_staidelta_config,
    initial_config_file,
    load_config,
    save_config,
    unflatten_properties,
)
from staidelta.util.ints import uint32
from staidelta.util.keychain import Keychain
from staidelta.util.path import mkdir
from staidelta.util.ssl_check import (
...
from staidelta.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
from staidelta.cmds.configure import configure
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'staidelta keys generate'")
        return None

    config: Dict = load_config(new_root, "config.yaml")
    pool_child_pubkeys = [master_sk_to_pool_sk(sk).get_g1() for sk, _ in all_sks]
    all_targets = []
    stop_searching_for_farmer = "staidelta_target_address" not in config["farmer"]
    stop_searching_for_pool = "staidelta_target_address" not in config["pool"]
    stop_searching_for_timelord = "staidelta_target_address" not in config["timelord"]
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("staidelta_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("staidelta_target_address"):
                stop_searching_for_pool = True
            if all_targets[-1] == config["timelord"].get("staidelta_target_address"):
                stop_searching_for_timelord = True

    # Set the destinations, if necessary
    updated_target: bool = False
    if "staidelta_target_address" not in config["farmer"]:
        print(
            f"Setting the staidelta destination for the farmer reward (1/8 plus fees, solo and pooling) to {all_targets[0]}"
        )
        config["farmer"]["staidelta_target_address"] = all_targets[0]
        updated_target = True
    elif config["farmer"]["staidelta_target_address"] not in all_targets:
        print(
            f"WARNING: using a farmer address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['farmer']['staidelta_target_address']} with {all_targets[0]}"
        )

    if "pool" not in config:
        config["pool"] = {}
    if "staidelta_target_address" not in config["pool"]:
        print(f"Setting the staidelta destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
        config["pool"]["staidelta_target_address"] = all_targets[0]
        updated_target = True
    elif config["pool"]["staidelta_target_address"] not in all_targets:
        print(
            f"WARNING: using a pool address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['pool']['staidelta_target_address']} with {all_targets[0]}"
        )
    if "timelord" not in config:
        config["timelord"] = {}
    if "staidelta_target_address" not in config["timelord"]:
        print(f"Setting the staidelta destination address for timelord reward to {all_targets[0]}")
        config["timelord"]["staidelta_target_address"] = all_targets[0]
        updated_target = True
    elif config["timelord"]["staidelta_target_address"] not in all_targets:
        print(
            f"WARNING: using a timelord address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['timelord']['staidelta_target_address']} with {all_targets[0]}"
        )
    if updated_target:
        print(
            f"To change the STAIDELTA destination addresses, edit the `staidelta_target_address` entries in"
...
def create_all_ssl(root_path: Path):
    ...
    staidelta_ca_crt, staidelta_ca_key = get_staidelta_ca_crt_key()
    staidelta_ca_crt_path = ca_dir / "staidelta_ca.crt"
    staidelta_ca_key_path = ca_dir / "staidelta_ca.key"
    write_ssl_cert_and_key(staidelta_ca_crt_path, staidelta_ca_crt, staidelta_ca_key_path, staidelta_ca_key)
    ...
    staidelta_ca_crt, staidelta_ca_key = get_staidelta_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, staidelta_ca_crt, staidelta_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path, fix_ssl_permissions: bool = False, testnet: bool = False):
    if create_certs is not None:
        if root_path.exists():
            ...
            if (
                staidelta_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet) == 0
        ...
        return staidelta_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet)


def staidelta_version_number() -> Tuple[str, str, str, str]:
...
def staidelta_minor_release_number():
    res = int(staidelta_version_number()[2])
    print(f"Install release number: {res}")
    return res


def staidelta_full_version_str() -> str:
    major, minor, patch, dev = staidelta_version_number()
...
def staidelta_init(
    ...
    """
    if os.environ.get("STAIDELTA_ROOT", None) is not None:
        print(
            f"warning, your STAIDELTA_ROOT is set to {os.environ['STAIDELTA_ROOT']}. "
            f"Please unset the environment variable and run staidelta init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"StaiDelta directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if STAIDELTA_ROOT is set, or if user has run staidelta init twice
        # before a new update.
        if testnet:
    ...
    if should_check_keys:
        check_keys(root_path)
    print("")
    print("To see your keys, run 'staidelta keys show --show-mnemonic-seed'")
