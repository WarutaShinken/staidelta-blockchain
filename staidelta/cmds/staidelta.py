...
from staidelta.cmds.configure import configure_cmd
from staidelta.cmds.farm import farm_cmd
from staidelta.cmds.init import init_cmd
from staidelta.cmds.keys import keys_cmd
from staidelta.cmds.netspace import netspace_cmd
from staidelta.cmds.passphrase import passphrase_cmd
from staidelta.cmds.plots import plots_cmd
from staidelta.cmds.show import show_cmd
from staidelta.cmds.start import start_cmd
from staidelta.cmds.stop import stop_cmd
from staidelta.cmds.wallet import wallet_cmd
from staidelta.cmds.plotters import plotters_cmd
from staidelta.util.default_root import DEFAULT_KEYS_ROOT_PATH, DEFAULT_ROOT_PATH
from staidelta.util.keychain import set_keys_root_path, supports_keyring_passphrase
from staidelta.util.ssl_check import check_ssl
...
@click.group(
    help=f"\n  Manage staidelta blockchain infrastructure ({__version__})\n",
    epilog="Try 'staidelta start node', 'staidelta netspace -d 192', or 'staidelta show -s'",
...
def cli(
    ...
    if passphrase_file is not None:
        from staidelta.cmds.passphrase_funcs import cache_passphrase, read_passphrase_from_file
...
if not supports_keyring_passphrase():
    from staidelta.cmds.passphrase_funcs import remove_passphrase_options_from_cmd

    # TODO: Remove once keyring passphrase management is rolled out to all platforms
    remove_passphrase_options_from_cmd(cli)


@cli.command("version", short_help="Show staidelta version")
def version_cmd() -> None:
    print(__version__)


@cli.command("run_daemon", short_help="Runs staidelta daemon")
@click.option(
    "--wait-for-unlock",
    help="If the keyring is passphrase-protected, the daemon will wait for an unlock command before accessing keys",
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by staidelta start <service>
)
@click.pass_context
def run_daemon_cmd(ctx: click.Context, wait_for_unlock: bool) -> None:
    import asyncio
    from staidelta.daemon.server import async_run_daemon
