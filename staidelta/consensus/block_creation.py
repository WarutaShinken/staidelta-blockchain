...
from staidelta.types.blockchain_format.reward_chain_block import RewardChainBlock, RewardChainBlockUnfinished
...
from staidelta.util.recursive_replace import recursive_replace
...
def create_foliage(
    ...
    if is_transaction_block:
        ...
        if height > 0:
            ...
            #Farmer difficulty coeff added in block creation : the amount of reward depends on the farmer staking amount
            farmer_coeff = blocks.get_farmer_difficulty_coeff_sync(
               curr.farmer_public_key, curr.height if curr.height > 0 else 0
            )
            pool_coin_reward: uint64 = Decimal(1 / farmer_coeff) * uint64(calculate_pool_reward(curr.height))
            farmer_coin_reward: uint64 = Decimal(1 / farmer_coeff) * uint64(calculate_base_farmer_reward(curr.height))
            ...
            reward_claims_incorporated += [pool_coin, farmer_coin, officialwallets_coin, timelord_coin]
            ...
            if curr.height > 0:
                ...
                while not curr.is_transaction_block:
                    ...
                    pool_coin_reward = Decimal(1 / farmer_coeff) * uint64(calculate_pool_reward(curr.height))
                    farmer_coin_reward = Decimal(1 / farmer_coeff) * uint64(calculate_base_farmer_reward(curr.height))
                    pool_coin = create_pool_coin(
                        curr.height,
                        curr.pool_puzzle_hash,
                        pool_coin_reward,
