...
from staidelta.consensus.block_rewards import calculate_base_officialwallets_reward, calculate_base_farmer_reward, calculate_pool_reward, calculate_base_timelord_fee
from staidelta.consensus.block_root_validation import validate_block_merkle_roots
from staidelta.full_node.mempool_check_conditions import mempool_check_conditions_dict
from staidelta.consensus.blockchain_interface import BlockchainInterface
from staidelta.consensus.coinbase import create_officialwallets_coin, create_farmer_coin, create_pool_coin, create_timelord_coin
from staidelta.consensus.constants import ConsensusConstants
from staidelta.consensus.cost_calculator import NPCResult, calculate_cost_of_program
...
from staidelta.full_node.mempool_check_conditions import get_name_puzzle_conditions
...
from staidelta.types.condition_opcodes import ConditionOpcode
from staidelta.types.condition_with_args import ConditionWithArgs
from staidelta.types.full_block import FullBlock
from staidelta.types.generator_types import BlockGenerator
from staidelta.types.name_puzzle_condition import NPC
...
from staidelta.util.generator_tools import (
    additions_for_npc,
...
async def validate_block_body(
    ...
    #1st part of Farmer coeff addition
    if height > 0:
        ...
        farmer_coeff = blocks.get_farmer_difficulty_coeff_sync(prev_transaction_block.farmer_public_key, prev_transaction_block.height if prev_transaction_block.height > 0 else 0)
        pool_coin_reward: uint64 = Decimal(1 / farmer_coeff) * uint64(calculate_pool_reward(prev_transaction_block.height))
        farmer_coin_reward: uint64 = Decimal(1 / farmer_coeff) * uint64(calculate_base_farmer_reward(prev_transaction_block.height))
        log.info(f"Farming block with Farmer Personal difficulty: {farmer_coeff}")
        ...
        farmer_coin = create_farmer_coin(
            ...
            (farmer_coin_reward + prev_transaction_block.fees),
            constants.GENESIS_CHALLENGE,
        )
        ...
        if prev_transaction_block.height > 0:
            ...
            while not curr_b.is_transaction_block:
                #2nd part of farmer coeff when 2nd block hit verify with farmer coeff
                farmer_coeff = blocks.get_farmer_difficulty_coeff_sync(curr_b.farmer_public_key, curr_b.height if curr_b.height > 0 else 0)
                _pool_coin_reward: uint64 = Decimal(1 / farmer_coeff) * uint64(calculate_pool_reward(curr_b.height))
                _farmer_coin_reward: uint64 = Decimal(1 / farmer_coeff) * uint64(calculate_base_farmer_reward(curr_b.height))
