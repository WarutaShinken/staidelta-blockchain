...
from staidelta.consensus.block_body_validation import validate_block_body
from staidelta.consensus.block_header_validation import validate_finished_header_block, validate_unfinished_header_block
...
from staidelta.consensus.multiprocess_validation import (
    PreValidationResult,
    pre_validate_blocks_multiprocessing,
    _run_generator,
)
from staidelta.consensus.pos_quality import UI_ACTUAL_SPACE_CONSTANT_FACTOR
...
from staidelta.types.blockchain_format.vdf import VDFInfo
...
from staidelta.types.generator_types import BlockGenerator, GeneratorArg
from staidelta.types.header_block import HeaderBlock
from staidelta.types.unfinished_block import UnfinishedBlock
from staidelta.types.unfinished_header_block import UnfinishedHeaderBlock
from staidelta.types.weight_proof import SubEpochChallengeSegment
from staidelta.util.errors import Err, ConsensusError
from staidelta.util.generator_tools import get_block_header, tx_removals_and_additions
from staidelta.util.ints import uint16, uint32, uint64, uint128
from staidelta.util.streamable import recurse_jsonify
...
class Blockchain(BlockchainInterface):
    ...
    async def get_farmer_difficulty_coeff(
        ...
        coeff = Decimal(0)
        space = 0
        if minimal_staking == 0:
            coeff = Decimal(20)
        else:
            space = uint128(int(network_space * blocks / block_range))
            if space > 0:
                staking_converted = (staking / 1000000000) + 0.0005
                farmer_space_converted = space / 1099511627776
                farmer_space = farmer_space_converted / 100
                ff = sqrt((100.26 * 1.0074 ** farmer_space) - 100)
                af = ff * farmer_space * 100
                sc = staking_converted / af
                sf1p = 0.05 + (1 / ((staking_converted / (af) + 0.05)))
                sf1m = (0.956 ** sc) * 1.046
                if sf1m < 0.8:
                    coeff = 0.8
                elif sc <= 1:
                ...
                staking_converted1 = (staking / 1000000000) + 0.0005
                ff1 = sqrt((100.26 * 1.0074 ** 1) - 100)
                af1 = ff1 * 100
                sc1 = (staking_converted1 / af1)
                sf1m1 = (0.873 ** sc1) * 1.046
                if sf1m1 < 0.8:
                    coeff = 0.8
                elif sc1 <= 1:
                    coeff = Decimal(1)
                ...
                coeff = Decimal(20)
        log.info(
             f"minimal_staking : {minimal_staking}, farmer space : {space} "
