...
DEFAULT_USER = f"user-staidelta-{CURRENT_KEY_VERSION}"  # e.g. user-staidelta-1.8
DEFAULT_SERVICE = f"staidelta-{DEFAULT_USER}"  # e.g. staidelta-user-staidelta-1.8
...
def supports_keyring_passphrase() -> bool:
    # Support can be disabled by setting STAIDELTA_PASSPHRASE_SUPPORT to 0/false
    return os.environ.get("STAIDELTA_PASSPHRASE_SUPPORT", "true").lower() in ["1", "true"]
...
def obtain_current_passphrase(prompt: str = DEFAULT_PASSPHRASE_PROMPT, use_passphrase_cache: bool = False) -> str:
    ...
    before failing.
    """
    from staidelta.cmds.passphrase_funcs import prompt_for_passphrase
