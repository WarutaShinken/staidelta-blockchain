...
from staidelta.pools.pool_config import PoolWalletConfig, load_pool_config, update_pool_config
from staidelta.pools.pool_wallet_info import (
    PoolWalletInfo,
    PoolSingletonState,
    PoolState,
    FARMING_TO_POOL,
...
from staidelta.protocols.pool_protocol import POOL_PROTOCOL_VERSION
...
from staidelta.pools.pool_puzzles import (
...
from staidelta.util.ints import uint8, uint32, uint64
from staidelta.wallet.derive_keys import (
    master_sk_to_pooling_authentication_sk,
    find_owner_sk,
)
from staidelta.wallet.sign_coin_spends import sign_coin_spends
...
class PoolWallet:
    ...
    async def create_new_pool_wallet_transaction(
        ...
        the same pooling puzzle. This puzzle is a `staidelta singleton` that is
    ...
    async def claim_pool_rewards(self, fee: uint64) -> TransactionRecord:
        ...
        for coin_record in unspent_coin_records:
            ...
            block: Optional[BlockRecord] = blockchain.try_block_record(blockchain.height_to_hash(coin_to_height_farmed[coin_record.coin]))
            # calculate farmer diff coeff at prev transaction block height - 1 from block won
            if block.prev_transaction_block_height is not None:
                prev_height = block.prev_transaction_block_height
            else:
                prev_height = 0
