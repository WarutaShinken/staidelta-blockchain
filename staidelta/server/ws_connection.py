...
from staidelta.cmds.init_funcs import staidelta_full_version_str
from staidelta.protocols.protocol_message_types import ProtocolMessageTypes
from staidelta.protocols.protocol_state_machine import message_response_ok
from staidelta.protocols.protocol_timing import INTERNAL_PROTOCOL_ERROR_BAN_SECONDS
from staidelta.protocols.shared_protocol import Capability, Handshake
from staidelta.server.outbound_message import Message, NodeType, make_msg
from staidelta.server.rate_limits import RateLimiter
...
from staidelta.util.ints import uint8, uint16

# Each message is prepended with LENGTH_BYTES bytes specifying the length
from staidelta.util.network import class_for_type, is_localhost

# Max size 2^(8*4) which is around 4GiB
LENGTH_BYTES: int = 4


class WSStaiDeltaConnection:
    """
    ...
    async def perform_handshake(self, network_id: str, protocol_version: str, server_port: int, local_type: NodeType):
        if self.is_outbound:
            outbound_handshake = make_msg(
                ...
                Handshake(
                    'staideltacoin-' + network_id,
                    protocol_version,
                    staidelta_full_version_str(),
                    uint16(server_port),
            ...
            if inbound_handshake.network_id != 'staideltacoin-' + network_id:
