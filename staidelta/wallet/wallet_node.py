...
from staidelta.daemon.keychain_proxy import (
    KeychainProxy,
...
from staidelta.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from staidelta.protocols import wallet_protocol, farmer_protocol
from staidelta.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
...
from staidelta.server.node_discovery import WalletPeers
...
from staidelta.server.ws_connection import WSStaiDeltaConnection
...
from staidelta.util.errors import Err, ValidationError
from staidelta.util.ints import uint32, uint64, uint128
from staidelta.util.keychain import Keychain
from staidelta.util.lru_cache import LRUCache
from staidelta.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
...
from staidelta.wallet.settings.settings_objects import BackupInitialized
...
from staidelta.wallet.wallet_blockchain import ReceiveBlockResult
from staidelta.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    server: Optional[StaiDeltaServer]
    log: logging.Logger
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'staidelta keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSStaiDeltaConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def update_stakings(self, peer: WSStaiDeltaConnection, height: uint64, farmer_public_key: G1Element) -> None:
        "fetch staking"
        ...
        self.wallet_state_manager.blockchain.coeff_at_height[bytes(res.stakings[0])][height]=Decimal(res.stakings[1])
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSStaiDeltaConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSStaiDeltaConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSStaiDeltaConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSStaiDeltaConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
...
async def wallet_next_block_check(
    peer: WSStaiDeltaConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:
