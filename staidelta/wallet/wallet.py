...
from staidelta.consensus.cost_calculator import calculate_cost_of_program, NPCResult
...
from staidelta.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_for_pk,
    solution_for_conditions,
)
from staidelta.wallet.puzzles.puzzle_utils import (
...
from staidelta.wallet.secret_key_store import SecretKeyStore
...
class Wallet:
    ...
    async def create_spend_bundle_relative_staidelta(self, staidelta_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None

        # If we're losing value then get coins with at least that much value
        # If we're gaining value then our amount doesn't matter
        if staidelta_amount < 0:
            utxos = await self.select_coins(abs(staidelta_amount), exclude)
        else:
        ...
        staidelta_amount = spend_value + staidelta_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": staidelta_amount}]
