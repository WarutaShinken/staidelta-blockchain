...
from staidelta.pools.pool_wallet import PoolWallet
from staidelta.pools.pool_wallet_info import create_pool_state, FARMING_TO_POOL, PoolWalletInfo, PoolState
from staidelta.protocols.protocol_message_types import ProtocolMessageTypes
from staidelta.server.outbound_message import NodeType, make_msg
from staidelta.simulator.simulator_protocol import FarmNewBlockProtocol
from staidelta.consensus.blockchain_interface import BlockchainInterface
from staidelta.protocols import full_node_protocol
from staidelta.types.blockchain_format.coin import Coin
from staidelta.types.blockchain_format.sized_bytes import bytes32
from staidelta.util.bech32m import decode_puzzle_hash, encode_puzzle_hash
from staidelta.util.byte_types import hexstr_to_bytes
from staidelta.util.ints import uint32, uint64
from staidelta.util.keychain import KeyringIsLocked, bytes_to_mnemonic, generate_mnemonic
from staidelta.util.path import path_from_root
from staidelta.util.ws_message import WsRpcMessage, create_payload_dict
from staidelta.wallet.cc_wallet.cc_wallet import CCWallet
from staidelta.wallet.derive_keys import master_sk_to_singleton_owner_sk
from staidelta.wallet.rl_wallet.rl_wallet import RLWallet
from staidelta.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_pool_sk, master_sk_to_wallet_sk
from staidelta.wallet.did_wallet.did_wallet import DIDWallet
from staidelta.wallet.trade_record import TradeRecord
from staidelta.wallet.transaction_record import TransactionRecord
from staidelta.wallet.util.backup_utils import download_backup, get_backup_info, upload_backup
from staidelta.wallet.util.trade_utils import trade_record_to_dict
from staidelta.wallet.util.transaction_type import TransactionType
from staidelta.wallet.util.wallet_types import WalletType
from staidelta.wallet.wallet_info import WalletInfo
from staidelta.wallet.wallet_node import WalletNode
from staidelta.util.config import load_config
from staidelta.consensus.coinbase import create_puzzlehash_for_pk, farmer_parent_id
...
class WalletRpcApi:
    def __init__(self, wallet_node: WalletNode):
        ...
        self.service_name = "staidelta_wallet"
    ...
    async def _state_changed(self, *args) -> List[WsRpcMessage]:
        ...
        return [create_payload_dict("state_changed", data, "staidelta_wallet", "wallet_ui")]
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("staidelta_target_address")
        pool_target = config["pool"].get("staidelta_target_address")
        found_farmer = False
        found_pool = False
    ...
    async def create_new_wallet(self, request: Dict):
        ...
        if request["wallet_type"] == "cc_wallet":
            ...
            if request["mode"] == "new":
                ...
                from staidelta.pools.pool_wallet_info import initial_pool_state_from_dict
    ...
    async def get_farmed_amount(self, request):
        ...
        for record in tx_records:
            ...
            if record.type == TransactionType.FEE_REWARD:
                for block_index in range(record.confirmed_at_height, record.confirmed_at_height - 100, -1):
                    if block_index < 0:
                        height = 0
                    else:
                        if farmer_parent_id(uint32(block_index), self.service.constants.GENESIS_CHALLENGE) == record.additions[0].parent_coin_info:
                            height = block_index
                blockchain = self.service.wallet_state_manager.blockchain
                # get block record from height farmed
                block: Optional[BlockRecord] = blockchain.try_block_record(blockchain.height_to_hash(height))
                # calculate farmer diff coeff at block won
                farmer_coeff = blockchain.get_farmer_difficulty_coeff_sync(
                    block.farmer_public_key, height if height > 0 else 0
                )
                base_reward = uint64(Decimal(1 / farmer_coeff) * calculate_base_farmer_reward(height))
                fee_amount += record.amount - base_reward
                farmer_reward_amount += base_reward
