...
from staidelta.plotters.bladebit import get_bladebit_install_info, plot_bladebit
from staidelta.plotters.chiapos import get_chiapos_install_info, plot_staidelta
from staidelta.plotters.madmax import get_madmax_install_info, plot_madmax
from staidelta.plotters.install_plotter import install_plotter
...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in STAIDELTA_ROOT.
    staidelta_root_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within STAIDELTA_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
    ...
    build_parser(subparsers, root_path, staidelta_plotter, "chiapos", "StaiDeltapos Plotter")
    ...
    if args.plotter == "chiapos":
        plot_staidelta(args, staidelta_root_path)
    if args.plotter == "madmax":
        plot_madmax(args, staidelta_root_path, root_path)
    if args.plotter == "bladebit":
        plot_bladebit(args, staidelta_root_path, root_path)
