...
from staidelta.cmds.init_funcs import check_keys, staidelta_init
from staidelta.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from staidelta.daemon.keychain_server import KeychainServer, keychain_commands
from staidelta.daemon.windows_signal import kill
from staidelta.plotters.plotters import get_available_plotters
from staidelta.plotting.util import add_plot_directory
from staidelta.server.server import ssl_context_for_root, ssl_context_for_server
from staidelta.ssl.create_ssl import get_mozilla_ca_crt
from staidelta.util.staidelta_logging import initialize_logging
from staidelta.util.config import load_config
from staidelta.util.json_util import dict_to_json_str
from staidelta.util.keychain import (
    Keychain,
...
from staidelta.util.service_groups import validate_service
from staidelta.util.setproctitle import setproctitle
from staidelta.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting StaiDelta.")
    quit()
...
service_plotter = "staidelta_plotter"
...
if getattr(sys, "frozen", False):
    name_map = {
        "staidelta": "staidelta",
        "staidelta_wallet": "start_wallet",
        "staidelta_full_node": "start_full_node",
        "staidelta_harvester": "start_harvester",
        "staidelta_farmer": "start_farmer",
        "staidelta_introducer": "start_introducer",
        "staidelta_timelord": "start_timelord",
        "staidelta_timelord_launcher": "timelord_launcher",
        "staidelta_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        command_args: List[str] = ["staidelta", "plotters", plotter]
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("staidelta daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered STAIDELTA_ROOT
    os.environ["STAIDELTA_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with STAIDELTA_ROOT: {os.environ['STAIDELTA_ROOT']}")
    ...
    if service_command == "staidelta_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in staidelta_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    staidelta_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("staidelta_daemon")
