...
from staidelta.daemon.server import WebSocketServer, create_server_for_daemon, daemon_launch_lock_path, singleton
from staidelta.full_node.full_node_api import FullNodeAPI
from staidelta.server.start_farmer import service_kwargs_for_farmer
from staidelta.server.start_full_node import service_kwargs_for_full_node
from staidelta.server.start_harvester import service_kwargs_for_harvester
from staidelta.server.start_introducer import service_kwargs_for_introducer
from staidelta.server.start_service import Service
from staidelta.server.start_timelord import service_kwargs_for_timelord
from staidelta.server.start_wallet import service_kwargs_for_wallet
from staidelta.simulator.start_simulator import service_kwargs_for_full_node_simulator
from staidelta.timelord.timelord_launcher import kill_processes, spawn_process
...
from staidelta.util.ints import uint16, uint32
from staidelta.util.keychain import bytes_to_mnemonic
...
async def setup_farmer(
    ...
    config["staidelta_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "staidelta")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["staidelta_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "staidelta")
