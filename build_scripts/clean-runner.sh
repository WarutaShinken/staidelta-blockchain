...
rm -rf ../venv || true
rm -rf venv || true
rm -rf staidelta_blockchain.egg-info || true
rm -rf build_scripts/final_installer || true
rm -rf build_scripts/dist || true
rm -rf build_scripts/pyinstaller || true
rm -rf staidelta-blockchain-gui/build || true
rm -rf staidelta-blockchain-gui/daemon || true
rm -rf staidelta-blockchain-gui/node_modules || true
rm staidelta-blockchain-gui/temp.json || true
( cd "$PWD/staidelta-blockchain-gui" && git checkout HEAD -- package-lock.json ) || true
cd "$PWD" || true
...
npm uninstall -g @staidelta-network/notarize-cli || true
