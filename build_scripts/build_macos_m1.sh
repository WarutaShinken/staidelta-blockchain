...
electron-packager . StaiDelta --asar.unpack="**/daemon/**" --platform=darwin \
...
if [ "$NOTARIZE" ]; then
  electron-osx-sign StaiDelta-darwin-arm64/StaiDelta.app --platform=darwin \
  --hardened-runtime=true --provisioning-profile=staideltablockchain.provisionprofile \
...
mv StaiDelta-darwin-arm64 ../build_scripts/dist/
cd ../build_scripts || exit

DMG_NAME="StaiDelta-$STAIDELTA_INSTALLER_VERSION-arm64.dmg"
echo "Create $DMG_NAME"
mkdir final_installer
electron-installer-dmg dist/StaiDelta-darwin-arm64/StaiDelta.app StaiDelta-$STAIDELTA_INSTALLER_VERSION-arm64 \
...
if [ "$NOTARIZE" ]; then
  ...
  notarize-cli --file=$DMG_NAME --bundle-id net.staidelta.blockchain \
...
# xcrun altool --notarize-app -f StaiDelta-0.1.X.dmg --primary-bundle-id net.staidelta.blockchain -u username -p password
...
# xcrun stapler staple StaiDelta-0.1.X.dmg
#
# Validate DMG:
# xcrun stapler validate StaiDelta-0.1.X.dmg
